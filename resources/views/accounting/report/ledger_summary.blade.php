        ...
        <div class="row">
            <div class="col-md-3">
                <div class="all-select-box">
                    <div class="btn-box">
                        {{ Form::label('start_date', __('Start Date'),['class'=>'text-xs text-primary']) }}
                        {{ Form::date('start_date',$filter['startDateRange'], array('class' => 'form-control')) }}
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="all-select-box">
                    <div class="btn-box">
                        {{ Form::label('end_date', __('End Date'),['class'=>'text-xs text-primary']) }}
                        {{ Form::date('end_date',$filter['endDateRange'], array('class' => 'form-control')) }}
                    </div>
                </div>
            </div>
            <div class="col-md-3">
    ...
    <div id="printableArea">
        <div class="row mt-4">
            <div class="col">
                ...
                <div class="card p-4 mb-4">
                    ...
                    <h6 class="text-muted mb-1">{{__('Ledger Summary')}}</h6>
                </div>
            </div>
        ...
        @if(!empty($account))
            <div class="row mt-4">
                <div class="col">
                    <div class="card p-4 mb-4">
                        <span class="h6 font-weight mb-0 ">{{__('Account Name')}} :</span>
                ...
                <div class="col">
                    <div class="card p-4 mb-4">
                        <span class="h6 font-weight mb-0 ">{{__('Account Code')}} :</span>
                        <h6 class="text-muted mb-1">{{$account->code}}</h6>
                    </div>
                </div>
                <div class="col">
                    <div class="card p-4 mb-4">
                        <span class="h6 font-weight mb-0 ">{{__('Total Debit')}} :</span>
                        <h6 class="text-muted mb-1">{{\Auth::user()->priceFormat($filter['debit'])}}</h6>
                    </div>
                </div>
                <div class="col">
                    <div class="card p-4 mb-4">
                        <span class="h6 font-weight mb-0 ">{{__('Total Credit')}} :</span>
                        <h6 class="text-muted mb-1">{{\Auth::user()->priceFormat($filter['credit'])}}</h6>
                    </div>
                </div>

                <div class="col">
                    <div class="card p-4 mb-4">
                        <span class="h6 font-weight mb-0 ">{{__('Balance')}} :</span>
                        <h6 class="text-muted mb-1">{{($filter['balance']>0)?__('Cr').'. '.\Auth::user()->priceFormat(abs($filter['balance'])):__('Dr').'. '.\Auth::user()->priceFormat(abs($filter['balance']))}}</h6>
                    </div>
                </div>
            </div>
        @endif
                ...
                <table class="table table-flush">
                    ...
                    @if(count($journalItems) > 0)
                        @foreach($journalItems as  $item)
                            <tr>
                                <td class="Id">
                                    <a href="{{ route('journal_entries.show',$item->journal) }}">{{ AUth::user()->journalNumberFormat($item->journal_id) }}</a>
                                </td>

                                <td>{{\App\Utility::getDateFormated($item->transaction_date)}}</td>
                                <td>{{\App\Utility::getDateFormated($item->created_at)}}</td>
                    ...
                    @else
                        <tr>
                            <td colspan="7" class="text-center">{{__('No Data Founds')}}</td>
                        </tr>
                    @endif
                    </tbody>
                </table>
