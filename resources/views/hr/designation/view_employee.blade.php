...
<div class="table-responsive">
    <table class="table align-items-center dataTable">
        ...
        <tbody class="list">
            @if ($designation->user_details->count() > 0)
                @foreach ($designation->user_details as $user_detail)
                    <tr>
                        <td>{{ $user_detail->user->name }} </td>
                        <td>{{ (!empty($designation->title) ?  $designation->title : '-') }}</td>
                        <td>{{ (!empty($user_detail->departmentDesc) ?  $user_detail->departmentDesc->title : '-') }}</td>
            ...
            @else
                <tr class="font-style">
                    <td colspan="6" class="text-center">
                        <h6 class="text-center">{{ __('No Employees Found.') }}</h6>
                    </td>
                </tr>
            @endif
        </tbody>
